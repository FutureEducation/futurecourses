...
class VideoAction extends Action{
    ...
    public function index(){
        ...
        if (!empty($_GET['q'])) {
            ...
            $this->assign("title","飞创教育 - 搜索结果——{$_GET['q']}");
        }
        //封装视频类型条件
        if (!empty($_GET['pid'])) {
            ...
            $this->assign("title","飞创教育 - ".$typename["name"]);
        }
        //拼装搜索用户上传的视频
        if ($_GET['uid']) {
            $where['uid'] = array("eq",$_GET['uid']);
            $this->assign("title","飞创教育 - 我上传的");
        }
        //拼装搜索用户收藏视频的id
        ...
        $page = new Page($total,6);//实例化一个分页对象
