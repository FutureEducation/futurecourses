...
class NoteAction extends Action{
    ...
    public function index(){
        ...
        if (!empty($_GET['q'])) {
            ...
            $this->assign("title","飞创教育 - 搜索结果");
        }
        //拼装搜索用户上传的手记
        if (!empty($_GET['uid'])) {
            $where['uid'] = array("eq",$_GET['uid']);
            $this->assign("title","飞创教育 - 我创建的");
        }
        //拼装指定视频相关的所有手记
        if (!empty($_GET['vid'])) {
            $where['vid'] = array("eq",$_GET['vid']);
            $videoname = D("Video")->field("videoname")->find($_GET['vid']);
            $this->assign("title","飞创教育 - 《{$videoname["videoname"]}》视频相关手记");
        }
        //拼装搜索某用户关于某视频的全部手记
        if (!empty($_GET['uid']) && !empty($_GET['vid'])) {
            ...
            $where['vid'] = array("eq",$_GET['vid']);
            $videoname = D("Video")->field("videoname")->find($_GET['vid']);
            $this->assign("title","飞创教育 - 我创建的关于《".$videoname["videoname"]."》的手记");
        }
        //拼装用户收藏的视频条件
        if (!empty($_GET['collid'])) {
            ...
            $this->assign("title","飞创教育 - 我收藏的手记列表");
        }
        //设置分页条件
    ...
    public function insert(){
        ...
        $list = $model->create();
        $list['title'] = trim(htmlspecialchars($list['title']));
